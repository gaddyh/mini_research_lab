"""
Refactored parameterized experiment runner using modular components.
"""
...
def create_experiment_json(exp_name: str, param_exp, result: dict, tables_dir: Path, figures_dir: Path) -> dict:
...
def create_family_summary_json(param_exp, results: dict, comparison_dir: Path) -> dict:
...
def main():
    """Main function using refactored modular components."""
    ...
    # Run first parameterized experiment (mean reversion with multiple lookbacks)
    ...
    for exp_name, result in results.items():
        ...
        copy_json_to_reports(tables_dir / "experiment.json", f"{exp_name}.json")
    ...
    copy_json_to_reports(comparison_dir / "family_summary.json", f"{param_exp.base_name}_family_summary.json")
    ...
    print(f"\n{'='*80}")
    print(f"📊 {param_exp.base_name.upper()} COMPARISON & DECISION SUMMARY")
    print(f"{'='*80}")
    ...
    with open(master_summary_path, "a") as f:
        f.write(f"\n=== {param_exp.base_name.upper()} ===\n")
        f.write(f"Variations: {len(results)}\n")
        for exp_name, result in results.items():
            reg = result["regression"].to_dict()
            f.write(f"  {exp_name}: coef={reg['coef']:+.4f}, p={reg['p_value']:.4f}, R²={reg['r_squared']:.4f}\n")
    
    print(f"\nMaster summary updated: {master_summary_path}")
